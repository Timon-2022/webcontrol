        ...
        async with async_playwright() as p:
            ...
            if not input_element:
                ...
                try:
                    ...
                    print(f"📊 页面中共有 {len(all_inputs)} 个输入类元素")
                    ...
                    for i, inp in enumerate(all_inputs):
                        ...
                        print(f"  {i+1}. {tag} - placeholder: '{placeholder}' - 可见: {visible} - 可用: {enabled}")
                except Exception as e:
                    print(f"❌ 分析页面结构失败: {e}")
                
                print("\n💡 建议:")
                print("  1. 检查是否需要完成额外的验证步骤")
                print("  2. 页面可能还在加载中")
                print("  3. 可能需要点击特定区域激活输入框")
